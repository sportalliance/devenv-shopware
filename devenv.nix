...
let
  ...
  importdbHelper = pkgs.writeScript "importdb" ''
        ...
        if [[ "${config.env.DATABASE_DUMP_URL}" == *.sql ]]; then
            curl --create-dirs "${config.env.DATABASE_DUMP_URL}" --output "$TARGETFOLDER/dump.sql"
        elif [[ "${config.env.DATABASE_DUMP_URL}" == *.sql.gz ]]; then
        ...
        elif [[ "${config.env.DATABASE_DUMP_URL}" == *.sql.zip ]]; then
...
in {
  ...
  scripts.importdb.exec = "${importdbHelper}";
}
